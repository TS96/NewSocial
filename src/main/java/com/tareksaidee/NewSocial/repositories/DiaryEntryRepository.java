...
public interface DiaryEntryRepository extends JpaRepository<DiaryEntry, Long> {
    ...
    @Query("SELECT d FROM DiaryEntry d WHERE d.user_name IN (SELECT f.friendshipId.username FROM Friendship f WHERE d.user_name = f.friendshipId.username AND f.request_status = \'approved\' AND f.friendshipId.friendName = :currentuser) ")
